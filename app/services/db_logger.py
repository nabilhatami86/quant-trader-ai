...
async def _tdb():
    ...
    """
    from app.database.session import make_thread_engine
...
async def _async_close_trade(ticket: int, close_price: float,
    ...
    from app.database.crud.trades import close_trade
    from app.database.crud.tx_log import log_event
    from app.database.crud.signals import update_signal_pnl
    try:
        async with _tdb() as db:
...
async def _async_get_trade_journal(symbol: str, limit: int) -> "pd.DataFrame":
    import pandas as pd
    from app.database.crud.trades import get_recent_trades
    try:
        async with _tdb() as db:
...
def _csv_journal_df(symbol: str, limit: int) -> "pd.DataFrame":
    ...
    from app.services.journal import get_recent_trades as csv_recent
...
async def _async_trade_stats(symbol: str) -> dict:
    from app.database.crud.trades import get_trade_stats
    try:
        async with _tdb() as db:
...
async def _async_load_candle_logs_df(symbol: str, timeframe: str, limit: int) -> "pd.DataFrame":
    from app.database.crud.candle_logs import get_candle_logs_df
    try:
        async with _tdb() as db:
...
async def _async_update_outcomes(symbol: str, timeframe: str, updates: list) -> int:
    from app.database.crud.candle_logs import update_outcomes_batch
    try:
        async with _tdb() as db:
