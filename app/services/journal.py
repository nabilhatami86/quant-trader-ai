...
Storage: data/journal/trade_journal.csv (satu file terpusat, semua symbol)
...
from app.core.paths import JOURNAL_DIR as _JOURNAL_DIR, TRADE_JOURNAL_PATH as _TRADE_JOURNAL_PATH

JOURNAL_DIR  = str(_JOURNAL_DIR)
JOURNAL_PATH = str(_TRADE_JOURNAL_PATH)
...
def _merge_with_db(symbol: str = "", limit: int = 30) -> pd.DataFrame:
    ...
    df_db = pd.DataFrame()
    try:
        from app.services.db_logger import get_trade_journal_df
