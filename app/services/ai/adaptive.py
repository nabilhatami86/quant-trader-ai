...
# STATE_PATH points to ai/adaptive_state.json at project root (backward compat)
# app/services/ai/ -> app/services/ -> app/ -> root -> ai/
_HERE = Path(__file__).parent
STATE_PATH = str(_HERE.parent.parent.parent / 'ai' / 'adaptive_state.json')
...
class AdaptiveLearner:
    ...
    def record_trade_outcome(
        ...
        # ── 0. Update signal_analysis.jsonl dengan outcome ───────────────────
        try:
            from app.services.ai.signal_logger import update_outcome as _upd_out
            _upd_out(ticket=ticket, outcome=result, pnl=pnl)
        except Exception:
            pass
    ...
    def _adjust_threshold(self):
        """MIN_SIGNAL_SCORE dikunci — tidak disesuaikan secara otomatis."""
        return
