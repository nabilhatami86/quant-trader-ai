...
from config import *
from app.services.candle_db import save_candles, merge_with_db, get_db_stats
...
try:
    ...
    from app.services.ai.deep_model import LSTMPredictor, TF_AVAILABLE
    from app.services.news.news_filter import NewsFilter
    AI_AVAILABLE = True
...
try:
    from app.services.ai.ml.predictor import ScalpingPredictor
...
class TradingBot:
    ...
    def load_data(self) -> bool:
        if self.mt5_conn:
            ...
            from app.services.tradingview.tv_feed import get_tv_ohlcv, TV_AVAILABLE
            if TV_AVAILABLE:
        ...
        try:
            from app.services.db_logger import save_candles_batch, save_candle_logs_batch
    ...
    def analyze(self, candle_memory: dict = None) -> dict:
        ...
        _sess_bias_str   = ""
        try:
            from app.services.session_bias import get_current_bias
        ...
        _plan_bias = "NEUTRAL"
        try:
            from app.services.session_bias import get_session_plan
        ...
        # ── Session Block — skip jam yang secara historis rugi ───────────────
        import datetime as _dt_sess
        try:
            from config import BLOCK_HOURS_WIB
            _wib_hour = (_dt_sess.datetime.utcnow().hour + 7) % 24
            if _wib_hour in BLOCK_HOURS_WIB:
                sc_dir = "WAIT"  # paksa WAIT di jam rugi
                print(f"  [SESSION-BLOCK] Jam {_wib_hour:02d}:xx WIB — "
                      f"jam ini historis rugi, skip trading")
        except Exception:
            pass
        ...
        if _n_open >= MAX_OPEN_POSITIONS:
            ...
            _block_reason  = ""
            try:
                from config import TREND_FILTER_STRONG_PROB
                _trend_override_prob = float(TREND_FILTER_STRONG_PROB)
            except Exception:
                _trend_override_prob = 0.75

            if _h1_trend == "BULL" and sc_dir == "SELL":
                if sc_prob < _trend_override_prob:
                    _trend_blocked = True
                    _block_reason  = f"H1_BULL_ADX{_h1_adx:.0f}"
                    print(f"  [TREND] H1 BULLISH kuat (ADX={_h1_adx:.1f}) "
                          f"— blok SELL (prob={sc_prob:.3f}), tunggu BUY")
                else:
                    print(f"  [TREND] H1 BULLISH kuat, tapi SELL prob={sc_prob:.3f} "
                          f">= {_trend_override_prob:.2f} — izinkan counter-trend kuat")
                ...
                if sc_prob < _trend_override_prob:
                    ...
                    print(f"  [TREND] H1 BEARISH kuat, tapi BUY prob={sc_prob:.3f} "
                          f">= {_trend_override_prob:.2f} — izinkan counter-trend kuat")
