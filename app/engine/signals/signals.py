...
def calculate_smart_tp_sl(direction: str, close: float, atr: float,
    ...
    try:
        from config import (
            FIXED_SL_PIPS, FIXED_TP_PIPS,
            ADAPTIVE_TP_BY_PROB,
            TP_PROB_MEDIUM, TP_PROB_STRONG,
            FIXED_TP_PIPS_WEAK, FIXED_TP_PIPS_MEDIUM, FIXED_TP_PIPS_STRONG,
        )
    ...
    if _use_fixed:
        ...
        # Jika final_score berupa probabilitas ML (0-1), pakai TP adaptif.
        # Tujuannya menambah TP hit rate pada sinyal medium tanpa
        # memotong reward trade yang benar-benar kuat.
        if 0.0 <= float(final_score) <= 1.0 and 'ADAPTIVE_TP_BY_PROB' in locals() and ADAPTIVE_TP_BY_PROB:
            _prob = float(final_score)
            if _prob >= float(TP_PROB_STRONG):
                tp_dist = float(FIXED_TP_PIPS_STRONG)
            elif _prob >= float(TP_PROB_MEDIUM):
                tp_dist = float(FIXED_TP_PIPS_MEDIUM)
            else:
                tp_dist = float(FIXED_TP_PIPS_WEAK)
