...
class MT5Connector:
    ...
    def place_order(self, symbol_key: str, direction: str,
        ...
        if result.retcode == mt5.TRADE_RETCODE_DONE:
            ...
            try:
                from app.services.journal import log_entry
                ...
                pass
            # Tandai signal log bahwa sinyal ini menghasilkan trade
            try:
                from app.services.ai.signal_logger import mark_trade_opened as _mto
                _last_sid = getattr(self, '_last_signal_id', None)
                if _last_sid:
                    _mto(_last_sid, result.order)
            except Exception:
                pass
            return {
                "success":   True,
    ...
    def close_position(self, ticket: int) -> dict:
        ...
        if result and result.retcode == mt5.TRADE_RETCODE_DONE:
            ...
            try:
                from app.services.journal import log_exit
...
class SignalExecutor:
    ...
    def should_trade(self, signal: dict, ml_pred: dict) -> bool:
        ...
        if self.real_mode:
            ...
            if not hasattr(self, "_daily_date") or self._daily_date != today:
                ...
                # Rekonstruksi dari journal CSV (handle bot restart di tengah hari)
                try:
                    ...
                    if _os.path.exists(JOURNAL_PATH):
                        ...
                        _td   = _jdf[_jdf.get("entry_time", _pd.Series(dtype=str))
                                        .str.startswith(_today_str, na=False)]
                        _pnl  = _pd.to_numeric(_td["pnl"], errors="coerce").fillna(0)
                        self._daily_gross_win = float(_pnl[_pnl > 0].sum())
                        self._daily_loss      = float(_pnl[_pnl < 0].abs().sum())
                        self._daily_profit    = float(_pnl.sum())
                    else:
    ...
    # ─────────────────────────────────────────────────────────────────────────
    def _pre_trade_check(self, direction: str, current_price: float,
                         ml_probability: float = 0.0) -> dict:
        """
        Koreksi analisis tepat sebelum order masuk.
        Ambil candle M5 terbaru langsung dari MT5, analisis kondisi:
          1. Trend EMA (EMA20 vs EMA50)
          2. Candle momentum terakhir 5c
          3. Posisi harga di range 20c
          4. Candle rejection (wick panjang melawan arah)
          5. Spread check
        ...
        try:
            # Ambil candle M5 terbaru via connector yang sudah dipakai bot
            _m5 = self.mt5.get_ohlcv(self.symbol, "5m", count=60)
            if _m5 is None or _m5.empty or len(_m5) < 20:
                return {
                    'go': True,
                    'score': 3,
                    'notes': ['Data candle tidak cukup — lewati cek'],
                    'warnings': [],
                    'verdict': 'GO',
                }

            _closes = _m5["Close"].to_numpy(dtype=float)
            _opens  = _m5["Open"].to_numpy(dtype=float)
            _highs  = _m5["High"].to_numpy(dtype=float)
            _lows   = _m5["Low"].to_numpy(dtype=float)
            ...
            _ema20 = _ema(_closes, 20)
            _ema50 = _ema(_closes, 50)
            _trend = 'UP' if _ema20 > _ema50 else 'DOWN'
            _trend_ok = (_trend == 'UP' and direction == 'BUY') or \
                        (_trend == 'DOWN' and direction == 'SELL')
            if _trend_ok:
                score += 1
                notes.append(f"Trend EMA20/50: {_trend} ✓ searah {direction}")
            else:
                warnings.append(f"Trend EMA: {_trend} berlawanan dengan {direction}")

            # 2. Candle momentum 5c terakhir
            _c5   = _closes[-5:]
            _up5  = sum(1 for i in range(1, 5) if _c5[i] > _c5[i-1])
            _dn5  = 4 - _up5
            _mom5 = 'NAIK' if _up5 >= 3 else ('TURUN' if _dn5 >= 3 else 'MIXED')
            _mom_ok = (_mom5 == 'NAIK' and direction == 'BUY') or \
                      (_mom5 == 'TURUN' and direction == 'SELL')
            if _mom_ok:
                score += 1
                notes.append(f"Momentum 5c: {_up5}↑/{_dn5}↓ ({_mom5}) ✓ searah {direction}")
            elif _mom5 == 'MIXED':
                score += 1
                notes.append(f"Momentum 5c: MIXED — netral")
            else:
                warnings.append(f"Momentum 5c: {_mom5} melawan {direction} ({_up5}↑/{_dn5}↓)")

            # 3. Posisi harga di range 20c — jangan beli di puncak, jual di dasar
            _h20  = float(_highs[-20:].max())
            _l20  = float(_lows[-20:].min())
            _pos  = (current_price - _l20) / (_h20 - _l20 + 1e-9)
            if direction == 'BUY' and _pos > 0.80:
                warnings.append(f"Harga di puncak range 20c ({_pos:.0%}) — BUY berisiko reversal")
            elif direction == 'SELL' and _pos < 0.20:
                warnings.append(f"Harga di dasar range 20c ({_pos:.0%}) — SELL berisiko reversal")
            else:
                score += 1
                notes.append(f"Posisi range 20c: {_pos:.0%} ({'baik untuk ' + direction})")

            # 4. Candle rejection — wick panjang melawan arah di 3c terakhir
            _last_c = _m5.iloc[-1]
            _body   = abs(float(_last_c['Close']) - float(_last_c['Open']))
            _rng    = float(_last_c['High']) - float(_last_c['Low'])
            if _rng > 0:
                _upper_wick = float(_last_c['High']) - max(float(_last_c['Close']), float(_last_c['Open']))
                _lower_wick = min(float(_last_c['Close']), float(_last_c['Open'])) - float(_last_c['Low'])
                if direction == 'BUY' and _upper_wick > _body * 1.5:
                    warnings.append("Upper wick panjang di candle terakhir — tekanan jual kuat")
                elif direction == 'SELL' and _lower_wick > _body * 1.5:
                    warnings.append("Lower wick panjang di candle terakhir — tekanan beli kuat")
                else:
                    score += 1
                    notes.append(f"Candle terakhir: tidak ada rejection wick melawan {direction}")

            # 5. Konsistensi candle 3c terakhir
            _last3_bull = sum(1 for i in range(-3, 0) if _closes[i] > _opens[i])
            _last3_bear = 3 - _last3_bull
            if direction == 'BUY' and _last3_bull >= 2:
                score += 1
                notes.append(f"3 candle terakhir: {_last3_bull} hijau — mendukung BUY")
            elif direction == 'SELL' and _last3_bear >= 2:
                score += 1
                notes.append(f"3 candle terakhir: {_last3_bear} merah — mendukung SELL")
            else:
                notes.append(f"3 candle terakhir: {_last3_bull} hijau / {_last3_bear} merah — campuran")

            # 6. Spread check sederhana
            try:
                from config import MAX_SPREAD_PIPS
                _si = self.mt5.get_symbol_info(self.symbol) or {}
                _ask = float(_si.get("ask", 0) or 0)
                _bid = float(_si.get("bid", 0) or 0)
                _point = float(_si.get("point", 0.01) or 0.01)
                if _ask > 0 and _bid > 0 and _point > 0:
                    _spread_pips = abs(_ask - _bid) / (_point * 10)
                    if _spread_pips > MAX_SPREAD_PIPS:
                        warnings.append(
                            f"Spread {_spread_pips:.2f} pip > batas {MAX_SPREAD_PIPS:.2f} pip"
                        )
                    else:
                        score += 1
                        notes.append(f"Spread {_spread_pips:.2f} pip masih aman")
            except Exception:
                pass

        except Exception as _e:
            notes.append(f"Pre-trade check error: {_e}")
            return {'go': True, 'score': 3, 'notes': notes, 'warnings': warnings, 'verdict': 'GO'}

        # Verdict
        if score >= 4:
            verdict = 'GO'
        elif score >= 2 and len(warnings) <= 1:
            verdict = 'CAUTION'
        elif score <= 1 or (len(warnings) >= 2 and score < 3):
            verdict = 'SKIP'
        else:
            verdict = 'CAUTION'

        # Jika probabilitas ML sangat kuat, jangan terlalu keras blok entry.
        try:
            from config import PRETRADE_STRONG_PROB
            if verdict == 'SKIP' and float(ml_probability) >= float(PRETRADE_STRONG_PROB):
                warnings.append(
                    f"ML prob {float(ml_probability):.3f} >= {float(PRETRADE_STRONG_PROB):.2f} — downgrade SKIP -> CAUTION"
                )
                verdict = 'CAUTION'
        except Exception:
            pass
    ...
    def execute(self, signal: dict, ml_pred: dict = None,
        ...
        try:
            _ml_prob = float((ml_pred or {}).get("probability", signal.get("probability", 0.0)) or 0.0)
        except Exception:
            _ml_prob = 0.0

        # ── Koreksi analisa tepat sebelum order ──────────────────────────────
        _ptc = (
            self._pre_trade_check(direction, price, ml_probability=_ml_prob)
            if direction in ("BUY", "SELL") and price > 0 else None
        )
        if _ptc:
            print(f"  [PRE-TRADE] verdict={_ptc['verdict']} score={_ptc['score']}/6")
            for _msg in _ptc.get("notes", [])[:3]:
                print(f"    + {_msg}")
            for _msg in _ptc.get("warnings", [])[:3]:
                print(f"    ! {_msg}")
            if not _ptc.get("go", True):
                return {
                    "success": False,
                    "skipped": True,
                    "reason": f"pre_trade_check:{_ptc.get('verdict','SKIP')}",
                }

        # ── ENFORCE FIXED SL/TP — override apapun dari signal ─────────────────
        # Ini jaminan SL/TP selalu sesuai config, tidak bisa di-override
        try:
            from config import FIXED_SL_PIPS, FIXED_TP_PIPS
            if FIXED_SL_PIPS and FIXED_SL_PIPS > 0 and price > 0:
                _sl_dist = float(FIXED_SL_PIPS)
                _tp_dist = float(FIXED_TP_PIPS) if FIXED_TP_PIPS > 0 else _sl_dist
                if direction == "BUY":
                    sl = round(price - _sl_dist, 2)
                    tp = round(price + _tp_dist, 2)
                else:
                    sl = round(price + _sl_dist, 2)
                    tp = round(price - _tp_dist, 2)
        except Exception:
        ...
        try:
            from app.services.journal import get_stats
        ...
        # Verdict CAUTION tidak membatalkan order, tapi perkecil ukuran.
        try:
            from config import PRETRADE_CAUTION_LOT_MULT
            if _ptc and _ptc.get("verdict") == "CAUTION" and lot and lot > 0:
                _old_lot = lot
                _mult = max(0.1, float(PRETRADE_CAUTION_LOT_MULT))
                lot = round(max(0.01, lot * _mult), 2)
                if lot < _old_lot:
                    print(f"  [PRE-TRADE] CAUTION — lot dikurangi {_old_lot:.2f} -> {lot:.2f}")
        except Exception:
            pass
    ...
    def sync_closed_positions(self) -> list[dict]:
        ...
        import datetime as dt
        from app.services.journal import log_exit, is_open
        try:
            from app.services.db_logger import close_trade_in_db
        except Exception:
            ...
            try:
                ...
                try:
                    ...
                    if os.path.exists(JOURNAL_PATH):
                        ...
                        from app.services.candle_log import LOG_DIR
