"""
Candle Database - Penyimpanan Permanen Semua Candle
====================================================
Setiap kali bot fetch data, candle baru disimpan ke CSV lokal.
Data lama tidak dihapus — terus bertambah = ML makin pintar.

Format file: data/history/{symbol}_{timeframe}.csv
"""
...
def save_candles(df: pd.DataFrame, symbol: str, timeframe: str) -> int:
    """
    Simpan candle baru ke database.
    Candle yang sudah ada (berdasarkan timestamp) tidak di-duplikat.
    Returns: jumlah candle baru yang ditambahkan
    """
    ...
    if os.path.exists(path):
        ...
        # Gabungkan, buang duplikat berdasarkan timestamp
...
def load_candles(symbol: str, timeframe: str) -> pd.DataFrame:
    """
    Load semua candle dari database lokal.
    Returns DataFrame kosong jika belum ada data.
    """
    path = _db_path(symbol, timeframe)
...
def merge_with_db(df_new: pd.DataFrame, symbol: str, timeframe: str) -> pd.DataFrame:
    """
    Gabungkan data baru dengan database historis.
    Returns DataFrame gabungan (historis + baru), sorted by time.
    Digunakan untuk training ML dengan data sebanyak mungkin.
    """
